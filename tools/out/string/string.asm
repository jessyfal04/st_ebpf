...
64 : instr(JMP(K,CALL(STATIC_ID)), dst=0, src=0, offset=0, imm=6) ~ info(trace_printk)
72 : instr(STX(DW,MEM), dst=10, src=0, offset=-24, imm=0) ~ ¤
80 : instr64(LD(DW,IMM), INTEGER, dst=1, src=0, offset=0, imm=3ll) ~ ¤
96 : instr(ALU(K,MOV), dst=2, src=0, offset=0, imm=25) ~ ¤
104 : instr(JMP(K,CALL(STATIC_ID)), dst=0, src=0, offset=0, imm=6) ~ info(trace_printk)
